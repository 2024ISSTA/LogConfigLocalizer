...
def verify_direct(content,anomaly_path):
    ...
    flag = False
    responses = ""
    confPath = r"LogConfigLocalizer/localizer/resources/property_corpus_numeric_only_with_des.csv"
    i=0
    hits = dict() 
...
def main(xml_file,records_path):
    ...
    for index,row in xml_content.iterrows():
        chosen_pr = row['name']
    # inject
    corpus_path = r"LogConfigLocalizer-submission/localizer/resources/property_corpus_numeric_only_with_des.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-cp",help="configuration file path")
    parser.add_argument("-af",help="anomaly_logs file path")
    args = parser.parse_args()
    if args.cp == None or args.af == None:
        print("configuration file path(-cp) and anomaly_logs file path(-af) are not to be empty!")
        sys.exit(1)
    main(args.cp,args.af)