...
# class DataEntry:
#     def __init__(self, option, value, log, possibility, explanation):
#         self.option = option
#         self.value = value
#         self.log = log
...
def gen_body(html_code,data):
    if len(data) == 0:
        ...
        for i, item in enumerate(data, start=1):
            ...
            """
    return html_code                         # <li style="background-color: #CD5C5C;">possibility: <strong>{item["possibility"]}</strong>;</li>
...
def get_recconfig(filename):
    ...
    #     filename = os.path.join(filename,'diagnosis_report.html')
    with open(filename, newline='') as csvfile:
        ...
        for row in csvreader:
            ...
            possibility = 1
            # print(f"Name: {name}, value: {value}, log: {log}, explanation: {explanation}")
            # print("\n")
